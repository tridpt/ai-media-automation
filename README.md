...
Bảng dưới cho biết chức năng nào đã kiểm chứng bằng dịch vụ thật, chức năng nào
mới chạy ở chế độ mock.

| Chức năng (theo yêu cầu đề) | Trạng thái |
|-----------------------------|-----------|
| Đọc task từ Google Sheets | ✅ chạy thật |
| Sinh media (PNG/JPG/GIF/MP3) bằng AI | ⚠️ code đầy đủ, demo bằng mock — cần credit OpenAI để tạo file thật |
| Lưu file lên Google Drive | ✅ chạy thật (OAuth, upload bằng quota tài khoản người dùng) |
| Thông báo Email (thành công/thất bại) | ✅ chạy thật |
| Thông báo Slack (thành công/thất bại) | ✅ chạy thật |
| Ghi log success/failure vào database | ✅ chạy thật |
| Báo cáo ngày + biểu đồ + email admin | ✅ chạy thật |

Khâu duy nhất chưa chạy thật là AI sinh media — do rào cản credit tài khoản OpenAI,
không phải thiếu code. Toàn bộ luồng đã được kiểm chứng đầy đủ qua chế độ mock (xem bên dưới).
...
└── src/
    ...
    ├── logger.py        # cấu hình logging ra console + file logs/
...
3. Tạo **Service Account**, tải file JSON key về đặt tên `service_account.json`
   (dùng để đọc Google Sheets).
4. Share Google Sheet cho email của service account (quyền Editor).
5. Cho phần **upload Drive**, tạo **OAuth Client ID (Desktop)**, tải về đặt tên
   `oauth_client.json`. Lần chạy đầu sẽ mở trình duyệt đăng nhập một lần rồi lưu
   `token.json`. Xem chi tiết trong `docs/SETUP_GOOGLE.md`.

> Vì sao Drive dùng OAuth chứ không dùng service account? Service account **không có
> quota lưu trữ** trên Drive cá nhân (báo lỗi `storageQuotaExceeded` khi upload). OAuth
> upload với tư cách tài khoản của bạn, dùng quota 15GB — nên khâu lưu trữ chạy thật được.
...
**7. Drive dùng OAuth thay vì service account.** Service account không có quota lưu trữ
trên Drive cá nhân (lỗi `storageQuotaExceeded`). Nên phần upload chuyển sang OAuth: đăng
nhập một lần bằng tài khoản của bạn, lưu `token.json`, các lần sau tự refresh. Service
account vẫn dùng để đọc Google Sheets. Có thể đổi qua `DRIVE_AUTH` trong `.env`.
